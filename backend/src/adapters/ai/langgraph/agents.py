...
async def attack_planner_agent(state: PentestState, db: Any) -> dict[str, Any]:
    ...
    system_prompt = (
        "You are a senior penetration tester. Given the recon data, generate realistic "
        "attack scenarios mapped to MITRE ATT&CK v15. Output valid JSON only.\n"
        "Schema: {\"scenarios\": [{\"id\": str, \"title\": str, \"tactic\": str, "
        "\"technique_id\": str, \"technique_name\": str, \"tools\": [str], "
        "\"preconditions\": [str], \"severity\": str, \"likelihood\": str, "
        "\"steps\": [str]}]}"
    )
    ...
    user_prompt = (
        ...
        f"Open ports:\n{ports_text}\n\n"
        f"Known CVEs:\n{cves_text}\n\n"
        "Generate 3–5 attack scenarios. Be realistic and focus on the highest-impact paths."
    )
    ...
    try:
        ...
        attack_scenarios = [
            AttackScenario(
                id="stub-001",
                title="Service Exploitation (stub)",
                tactic="Initial Access",
                technique_id="T1190",
                technique_name="Exploit Public-Facing Application",
                tools=["nuclei", "metasploit"],
                preconditions=["Reachable target", "Known CVE"],
                severity="high",
                likelihood="medium",
                steps=["Enumerate services", "Match CVEs", "Exploit", "Escalate"],
            )
        ]
    ...
    ptt = {
        ...
        "scenarios": [s for s in attack_scenarios],
    }
...
async def reporter_agent(state: PentestState, db: Any) -> dict[str, Any]:
    ...
    system_en = (
        "You are a cybersecurity report writer. Write a concise executive summary in English. "
        "Output JSON: {\"headline\": str, \"summary\": str, \"key_findings\": [str], "
    ...
    user_en = (
        ...
        f"KEV vulnerabilities: {kev_count}\n"
        f"Top findings: {chr(10).join(f'- {f}' for f in top_findings)}"
    )
    ...
    system_pl = (
        "Jesteś ekspertem ds. cyberbezpieczeństwa. Napisz zwięzłe podsumowanie wykonawcze po polsku. "
        "Odpowiedź w JSON: {\"headline\": str, \"summary\": str, \"key_findings\": [str], "
        "\"recommended_actions\": [str]}"
    )
    await _emit(state.get("scan_id"), "llm_phase", phase="PL executive summary")
    raw_pl = await _ollama_chat(settings.pentest_llm_reporter_model, system_pl, user_en, scan_id=state.get("scan_id"))
    ...
    try:
        data_pl = json.loads(raw_pl)
        exec_summary_pl = data_pl.get("summary", "Test penetracyjny zakończony.")
    except Exception:
        exec_summary_pl = "Test penetracyjny zakończony. Przejrzyj szczegółowe wyniki."
