...
async def _extract_profile(page: Any, username: str) -> IgProfile | None:
    ...
    media_count = None
    if og_desc:
        # Handles plain numbers "1,234" and suffixed "664M", "1.2K"
        f_m = re.search(r'([\d,.]+[KkMmBb]?)\s+Followers?', og_desc, re.IGNORECASE)
        if f_m:
            follower_count = _parse_count(f_m.group(1))
        fo_m = re.search(r'([\d,.]+[KkMmBb]?)\s+Following', og_desc, re.IGNORECASE)
        if fo_m:
            following_count = _parse_count(fo_m.group(1))
        p_m = re.search(r'([\d,.]+[KkMmBb]?)\s+Posts?', og_desc, re.IGNORECASE)
        if p_m:
    ...
    # biography from JSON blobs only — the OG description is counts text, not the bio
    # (Instagram's og:description = "X Followers, Y Following, Z Posts - See Instagram...")
