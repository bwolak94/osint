...
import { useLinkedInIntelHistory, useDeleteLinkedInIntelScan } from "../hooks";

interface Props {
  onSelect: (scan: LinkedInIntelScan) => void;
}

export function LinkedInHistory({ onSelect }: Props) {
  const [page, setPage] = useState(1);
  const { data, isLoading } = useLinkedInIntelHistory(page);
  const deleteMutation = useDeleteLinkedInIntelScan();
    ...
    <div className="space-y-2">
      ...
      {data.total_pages > 1 && (
        <div className="flex justify-center gap-2 pt-2">
          ...
          <span className="px-3 py-1 text-xs" style={{ color: "var(--text-tertiary)" }}>
