...
import type { NewsArticle } from "../components/NewsBoard";

// Stub useTranslation so component renders without i18n setup
...
describe("NewsBoard", () => {
  const articles: NewsArticle[] = [
    ...
    },
    {
      ...
      action_relevance_score: 0.8,
    },
  ];

  it("renders article titles", () => {
    render(<NewsBoard articles={articles} />);
    expect(screen.getByText("AI Breakthrough")).toBeDefined();
    expect(screen.getByText("Security Alert")).toBeDefined();
  });

  it("renders no_articles key when empty", () => {
    render(<NewsBoard articles={[]} />);
    expect(screen.getByText("no_articles")).toBeDefined();
  });

  it("renders credibility bar for articles with credibility_score", () => {
    const { container } = render(<NewsBoard articles={articles} />);
    // CredibilityBar renders a progress div
  ...
  it("renders external link for articles with url", () => {
    render(<NewsBoard articles={articles} />);
