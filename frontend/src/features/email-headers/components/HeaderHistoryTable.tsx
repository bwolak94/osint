...
import { useEmailHeaderHistory, useDeleteEmailCheck } from '../hooks'
import type { EmailHeaderCheck } from '../types'

interface Props {
  onSelect: (check: EmailHeaderCheck) => void
}
...
export function HeaderHistoryTable({ onSelect }: Props) {
  const [page, setPage] = useState(1)
  const [pageSize] = useState(10)
  const [pendingDelete, setPendingDelete] = useState<EmailHeaderCheck | null>(null)

  const { data, isLoading } = useEmailHeaderHistory(page, pageSize)
  const deleteMutation = useDeleteEmailCheck()
    ...
    <>
      <Card>
                ...
                : data?.items.map((check) => (
                    <tr key={check.id} className="border-b transition-colors hover:bg-bg-overlay" style={{ borderColor: 'var(--border-subtle)' }}>
                      <td className="px-4 py-3">
                        <span className="block max-w-[200px] truncate text-sm font-medium" style={{ color: 'var(--text-primary)' }}>{check.subject || '(no subject)'}</span>
                      </td>
                      <td className="px-4 py-3 text-xs" style={{ color: 'var(--text-secondary)' }}>
                        <span className="block max-w-[150px] truncate">{check.sender_from || '—'}</span>
                      </td>
                      <td className="px-4 py-3 font-mono text-xs" style={{ color: 'var(--text-secondary)' }}>{check.originating_ip || '—'}</td>
                      <td className="px-4 py-3">
                        {check.is_spoofed
                          ? <Badge variant="danger" size="sm">Spoofed</Badge>
        ...
        {!isLoading && (!data || data.items.length === 0) && (
          <CardBody>
            <EmptyState variant="no-data" title="No analysis history" description="Paste email headers to analyze them" />
          </CardBody>
        )}
      ...
      {pendingDelete && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4" role="dialog" aria-modal="true">
          ...
          <div className="relative z-10 w-full max-w-md rounded-xl border p-6 shadow-xl" style={{ background: 'var(--bg-surface)', borderColor: 'var(--border-subtle)' }}>
            ...
            <p className="mb-6 text-sm" style={{ color: 'var(--text-secondary)' }}>Delete this email header analysis? This cannot be undone.</p>
