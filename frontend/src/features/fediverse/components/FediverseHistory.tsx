...
import { useFediverseHistory, useDeleteFediverseScan } from '../hooks'
import type { FediverseScan } from '../types'

interface Props {
  onSelect: (scan: FediverseScan) => void
}
...
export function FediverseHistory({ onSelect }: Props) {
  const [page, setPage] = useState(1)
  const [pageSize, setPageSize] = useState<PageSizeOption>(10)
  const [pendingDelete, setPendingDelete] = useState<FediverseScan | null>(null)

  const { data, isLoading } = useFediverseHistory(page, pageSize)
  const deleteMutation = useDeleteFediverseScan()
                ...
                : data?.items.map((scan) => (
                    ...
                    >
                      ...
                      <td className="px-4 py-3 text-sm" style={{ color: 'var(--text-secondary)' }}>
                        {scan.platforms_searched.length}
                      </td>
                      <td className="px-4 py-3 text-sm" style={{ color: 'var(--text-secondary)' }}>
                        {scan.total_results}
                      </td>
        ...
        {!isLoading && (!data || data.items.length === 0) && (
          <CardBody>
            <EmptyState
              ...
              description="Search for Fediverse profiles to see them here"
            />
          </CardBody>
        )}
