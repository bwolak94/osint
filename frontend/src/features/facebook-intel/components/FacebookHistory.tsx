...
import { useFacebookIntelHistory, useDeleteFacebookIntelScan } from '../hooks'
import type { FacebookIntelScan } from '../types'

interface Props {
  onSelect: (scan: FacebookIntelScan) => void
}
...
function ConfirmDeleteDialog({
  ...
  return (
    <div
      ...
      role="dialog"
      aria-modal="true"
      aria-labelledby="fb-delete-dialog-title"
    >
      <div
...
export function FacebookHistory({ onSelect }: Props) {
  const [page, setPage] = useState(1)
  const [pageSize, setPageSize] = useState<PageSizeOption>(10)
  const [pendingDelete, setPendingDelete] = useState<FacebookIntelScan | null>(null)

  const { data, isLoading } = useFacebookIntelHistory(page, pageSize)
  const deleteMutation = useDeleteFacebookIntelScan()
        ...
        {!isLoading && (!data || data.items.length === 0) && (
          <CardBody>
            <EmptyState
              ...
              description="Search for Facebook profiles to see them here"
            />
          </CardBody>
        )}
