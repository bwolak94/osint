...
import { useCloudExposureHistory, useDeleteCloudScan } from '../hooks'
import type { CloudExposureScan } from '../types'

interface Props {
  onSelect: (scan: CloudExposureScan) => void
}
...
export function CloudScanHistory({ onSelect }: Props) {
  const [page, setPage] = useState(1)
  const [pageSize] = useState(10)
  const [pendingDelete, setPendingDelete] = useState<CloudExposureScan | null>(null)

  const { data, isLoading } = useCloudExposureHistory(page, pageSize)
  const deleteMutation = useDeleteCloudScan()
    ...
    <>
      <Card>
              ...
              {isLoading
                ...
                : data?.items.map((scan) => (
                    <tr key={scan.id} className="border-b transition-colors hover:bg-bg-overlay" style={{ borderColor: 'var(--border-subtle)' }}>
                      ...
                      <td className="px-4 py-3 text-sm" style={{ color: 'var(--text-secondary)' }}>{scan.total_buckets}</td>
                      <td className="px-4 py-3">
                        <Badge variant={scan.public_buckets > 0 ? 'warning' : 'success'} size="sm">{scan.public_buckets}</Badge>
                      </td>
                      <td className="px-4 py-3">
                        <Badge variant={scan.sensitive_findings > 0 ? 'danger' : 'success'} size="sm">{scan.sensitive_findings}</Badge>
                      </td>
        ...
        {!isLoading && (!data || data.items.length === 0) && (
          <CardBody>
            <EmptyState variant="no-data" title="No scan history" description="Enter a domain or org name to scan for exposed buckets" />
          </CardBody>
        )}
