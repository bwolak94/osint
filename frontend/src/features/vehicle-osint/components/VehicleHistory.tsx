...
import { useVehicleOsintHistory, useDeleteVehicleOsintScan } from "../hooks";

interface Props {
  onSelect: (scan: VehicleOsintScan) => void;
}

export function VehicleHistory({ onSelect }: Props) {
  const [page, setPage] = useState(1);
  const { data, isLoading } = useVehicleOsintHistory(page);
  const deleteMutation = useDeleteVehicleOsintScan();
      ...
      {data.items.map((scan) => (
        ...
        >
          <div className="min-w-0">
            <span className="font-medium font-mono truncate block" style={{ color: "var(--text-primary)" }}>
              {scan.query}
            </span>
